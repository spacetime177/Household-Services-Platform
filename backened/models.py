...
#db.model is inheritance in flask
#First entity
class User_Info(db.Model):
    __tablename__="user_info"
    id=db.Column(db.Integer,primary_key=True)
    email=db.Column(db.String,unique=True,nullable=False)  #not blank
    password=db.Column(db.String,nullable=False)        #not blank
    role=db.Column(db.Integer,default=1)    #all are users so default is 1 and the first one is admin
